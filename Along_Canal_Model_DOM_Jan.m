function Conc = Along_Canal_Model_DOM_Jan (p, X)
...
% deep_coef ; B ; V_mic ; V_atm_init ; V_atm_slope ; k_DOC 
deep_coef = p(1);
B = p(2);
V_mic_coef = p(3);
V_atm_init = p(4);
...
% M_12gw = deep_coef *(deep_gw(3,1)) + (1 - deep_coef)*(shallow_gw(3,1));  
% M_13gw = deep_coef *(deep_gw(4,1)) + (1 - deep_coef)*(shallow_gw(4,1)); 
% C_12gw = deep_coef *(deep_gw(1,1)) + (1 - deep_coef)*(shallow_gw(1,1)); 
% C_13gw = deep_coef *(deep_gw(2,1)) + (1 - deep_coef)*(shallow_gw(2,1));
...
x_calc_init = [0.01;1;2;3;4;5;10;20;30;40;50;100;200];
x_calc_init = [0.01;1;2;3;4;5;10;20;30;40;50;100;200;300;400];
%x_calc = [x_calc_init ; x_obs];
X = X';
x_calc = [x_calc_init ; X];

% Solve the ODE at certain values of x:
[~,Conc_a] = ode45(dC_dx , x_calc , y0); %[x,y] = ode23(odefun,xspan,y0)
%[Conc_a] = ode45(dC_dx , x_calc , y0); %[x,y] = ode23(odefun,xspan,y0)

Conc_a = Conc_a ((length(x_calc_init)+1):end,:); %Cut off the spin-up
...
%Re-scale the simulated concentrations using observed data
Conc_CO2_sc = (Conc_CO2 - min(Conc_obs_d_Jan(:,1)))/(max(Conc_obs_d_Jan(:,1)) - min(Conc_obs_d_Jan(:,1)));
delta_13CO2_sc = (delta_13CO2 - min(Conc_obs_d_Jan(:,2)))/(max(Conc_obs_d_Jan(:,2)) - min(Conc_obs_d_Jan(:,2)));
Conc_CH4_sc = (Conc_CH4 - min(Conc_obs_d_Jan(:,3)))/(max(Conc_obs_d_Jan(:,3)) - min(Conc_obs_d_Jan(:,3)));
delta_13CH4_sc = (delta_13CH4 - min(Conc_obs_d_Jan(:,4)))/(max(Conc_obs_d_Jan(:,4)) - min(Conc_obs_d_Jan(:,4)));
